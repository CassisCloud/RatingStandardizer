...
public sealed class ServerEntryPoint : IHostedService, IDisposable
{
    ...
    private async Task ProcessItemChangeAsync(ItemChangeEventArgs e)
    {
        try
        {
            ...
            var libraryProfile = LibraryFilter.FindProfile(e.Item, configuration.LibraryProfiles);
            var effectiveConfiguration = RatingConfigurationResolver.Resolve(configuration, libraryProfile);
            var result = ItemRatingStandardizer.Apply(e.Item, effectiveConfiguration, _ratingConverter, Plugin.HistoryStore, Plugin.PluginVersion);
            if (!result.MatchedMapping)
            {
                _logger.LogInformation(
                    "Skipped item {ItemName}. OfficialRating={OfficialRating}, Reason={Reason}, Preset={Preset}, TargetLibraryCount={TargetLibraryCount}.",
                    e.Item.Name,
                    e.Item.OfficialRating,
                    result.Status,
                    effectiveConfiguration.PresetId,
            ...
            _logger.LogInformation(
                "Standardized OfficialRating for {ItemName} from {OriginalRating} to {TargetRating}. Lock added: {LockAdded}, original tag added: {TagAdded}, history saved: {HistorySaved}",
                e.Item.Name,
